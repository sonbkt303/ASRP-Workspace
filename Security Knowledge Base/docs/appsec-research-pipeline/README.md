...
Khi bạn chat một AppSec topic (ví dụ: `HTTP caching`, `OAuth token validation`, `SSRF defense`), pipeline sẽ điều phối “team security” theo vai trò, và hệ thống hóa thành **Knowledge Base topic(s)** trong `Repository Content/knowledge/`.
...
1. KB dedup search trong `Repository Content/knowledge/` (xem `../../knowledge/README.md`)
...
Trước khi tạo KB topic mới, kiểm tra `Repository Content/knowledge/` (ví dụ `Repository Content/knowledge/web/HTTP.md`).
Nếu overlap đáng kể:
