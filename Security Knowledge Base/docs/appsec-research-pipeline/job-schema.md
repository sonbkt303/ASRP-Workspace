...
Runtime source of truth cho execution: `../../../../cursor-agents/skills/appsec-research-orchestrator/SKILL.md`. Mirror local: `../../../.cursor/skills/appsec-research-orchestrator/SKILL.md`.
...
output_mode: "propose file path"  # chat-only | propose file path | write to Repository Content/knowledge/
...
dedup_hits:
  - path: "Repository Content/knowledge/web/HTTP.md"
...
subtopics:
  - id: "<short-id>"
    ...
    proposed_path: "Repository Content/knowledge/web/http-caching-auth.md"
...
- `proposed_path` = `Repository Content/knowledge/<folder>/<kebab-case>.md`
- `category` frontmatter có thể dùng slash (`networking/http`); **folder** luôn là segment đầu tiên (`networking/`).
- Ví dụ: `category: networking/http` → `Repository Content/knowledge/networking/tcp-overview.md`
